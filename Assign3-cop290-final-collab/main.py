...
class User(db.Model):
    ...
    ProfilePhoto = db.Column(db.String(80), nullable=False)
...
def home():
    ...
    return render_template("index.html" ,lat_post = lat_post , recipe = recipe)
...
def all_posts():
    ...
    if (page==1):
        ...
        prev = "?page=" + str(page - 1)
        next = "?page=" + str(page + 1)
    return render_template("all_posts.html", all_posts=all_posts, prev=prev, next=next)
...
def post_details(post_slug):
    ...
    if(request.method=='POST'):
        ...
        db.session.add(entry)
        db.session.commit()
    return render_template('post-details.html', params=params, post=post, post_review = post_review)
...
def recipe_details(rec_slug):
    ...
    return render_template("recipe-details.html",recipe=recipe,all_rec = all_rec)
...
def search_recipe():
    ...
    return render_template("recipes.html",all_rec = found_rec)
...
def recipes():
    ...
    if (page==1):
        ...
        prev = "?page=" + str(page - 1)
        next = "?page=" + str(page + 1)
    return render_template("recipes.html", all_rec=all_rec, prev=prev, next=next)
...
def profile(username):
    ...
    if(request.method=='GET'):
        ...
        if (n_pass == nr_pass):
            search_name.password = generate_password_hash(request.form.get('password'), method='sha256')
            db.session.commit()
        else:
